...
def transform_to_joint(joint_state):
    ...
    time.time = joint_state.header.stamp.secs + 1e-9*joint_state.header.stamp.nsecs
    return np.array(joint), np.array(time.time)
...
def multi_callback(sub_torso, reference_traj, reference_stiff, initial_pose, torso_pub, time_array, index_counter):
    ...
    if index <= 18299:
        ...
        left_pose = robot_left_position_init
        right_stiff = reference_stiff[index, :]

        robot_right_pose_matrix = np.r_[
            np.c_[robot_right_rotation_matrix_init, right_pose[:3].T], np.array([[0, 0, 0, 1]])]
        robot_left_pose_matrix = np.r_[
            np.c_[robot_left_rotation_matrix_init, left_pose.T], np.array([[0, 0, 0, 1]])]
        ...
        base2torso_matrix = T_MobileBaseToTorsoBase @ T_TorsoBaseToTorsoEnd
        ...
        # 获取关节速度参数
        joint1_vel_ = rospy.get_param("joint1_vel", 0.07)
        joint3_vel_ = rospy.get_param("joint3_vel", 0.08)
...
if __name__ == '__main__':
    ...
    reference_traj = np.load('/home/curi/Chenzui/TASE/data/traj_box_carrying_18300.npy')
    reference_stiff = np.load('/home/curi/Chenzui/TASE/data/stiff_box_carrying_18300.npy')
